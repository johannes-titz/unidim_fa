...
```{r}
ggplot(grid_row, aes(phi, errors/n) + geom_point(alpha=0.2) +
  theme_classic()
```

Warum es keine gute Idee ist nur die rohen Fehler zu nehmen. Maximale Fehleranzahl ist gerade mal 6.25%:

```{r}
ggplot(grid_row, aes(phi, Var2*Var3/n^2)) + geom_point(alpha=0.2) +
  theme_classic()
```
...
get_hz <- function(d) {
  ...
  return(c(cor, hz, tetrachor = tetrachor))
}

res <- xfun::cache_rds({
  pblapply(seq(nrow(test)), function(x) get_hz(create_d(test[x, ])), cl = cores)
}, file = "sim2")

#l2 <- l[!is.na(l)] # omit NA

resm <- matrix(unlist(res), ncol = 3, byrow = T)
